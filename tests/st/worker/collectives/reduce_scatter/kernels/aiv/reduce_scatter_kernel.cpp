 ...
 */
/**
 * ReduceScatter kernel — symmetric, 4-phase, HCCL-window scratch pattern.
 *
 * Phase 1 (stage-in):   for chunk in 0..nranks-1: input[chunk*C..(chunk+1)*C) → scratch[chunk*C..)
 * Phase 2 (barrier):    signal matrix + TWAIT cross-rank sync
 * Phase 3 (reduce):     acc = scratch[my_rank*C]; for each peer: acc += peer's scratch[my_rank*C]
 * Phase 4 (stage-out):  TSTORE(output, acc)
 *
 * Each rank is responsible for reducing the chunk at index my_rank across all
 * ranks.  The signal area lives at the tail of the scratch buffer, after the
 * full nranks*COUNT_PER_RANK float staging area.
 *
 * args layout:
...
extern "C" __aicore__ __attribute__((always_inline)) void kernel_entry(__gm__ int64_t *args) {
    ...
    // Signal area: nranks int32 slots at the tail of the staging area.
    ...
    TileData accTile(1, COUNT_PER_RANK);
    ...
    // ------------------------------------------------------------------
    // Phase 1: stage-in — copy all N input chunks into scratch so that
    // every peer can read our entire input in Phase 3.
    // ------------------------------------------------------------------
    for (int chunk = 0; chunk < nranks; ++chunk) {
        Global inputChunkG(input + chunk * COUNT_PER_RANK, shape, stride);
        Global scratchChunkG(scratch + chunk * COUNT_PER_RANK, shape, stride);
    ...
    // CommRemotePtr (same pattern as allreduce Phase 3).
    // ------------------------------------------------------------------
    Global myChunkG(scratch + my_rank * COUNT_PER_RANK, shape, stride);
    ...
    for (int peer = 0; peer < nranks; ++peer) {
        if (peer == my_rank) continue;
        __gm__ float *remote_chunk = CommRemotePtr(commCtx, scratch + my_rank * COUNT_PER_RANK, peer);
