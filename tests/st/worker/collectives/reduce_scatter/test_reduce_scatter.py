...
# -----------------------------------------------------------------------------------------------------------
"""Scene test for distributed reduce_scatter.
...
def reduce_scatter_orch_fn(orch, callables, task_args, config):
...
_CALLABLE = {
    "orchestration": reduce_scatter_orch_fn,
    "callables": [
        {
            "name": "reduce_scatter",
            "orchestration": {
                "source": "kernels/orchestration/reduce_scatter_orch.cpp",
                "function_name": "reduce_scatter_orchestration",
            ...
            "incores": [
                {
                    "func_id": 0,
                    "source": "kernels/aiv/reduce_scatter_kernel.cpp",
                    "core_type": "aiv",
                    "signature": [D.IN, D.OUT, D.INOUT],
                }
            ],
